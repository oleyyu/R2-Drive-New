...
3. **一键卸载**：核对目标后，永久删除当前实例的 Worker、R2 文件与桶、D1、域名绑定和本地配置；若 R2 返回 `10008`，会启动临时远程清理 Worker，清除普通对象和残留分片上传后重试。
...
第 3 项只处理当前 `wrangler.jsonc` 明确指向的实例。执行前会显示 Worker、桶、数据库和域名，并要求手动输入 `DELETE`；随后先核对 Cloudflare 账号和 D1 编号，再删除 Worker 以阻止新的写入，最后删除 R2、D1 和本机配置。

R2 桶删除后，其中的全部对象、CORS、生命周期、事件通知和其他桶级配置都会一并删除，无法恢复。Worker 的版本、Secret 和域名路由也会随 Worker 删除。任一步失败时，启动器都会保留 `wrangler.jsonc` 中的目标信息以便重试；它不会退出 Wrangler 登录，不会删除 Cloudflare 账号、域名专区或其他项目，也不会代替你删除在 Cloudflare 后台手动创建、可能被其他项目共用的 R2 API Token。
