...
- 多账号存储池：用 Wrangler 把自己拥有的其他 Cloudflare R2 接成独立存储节点，按整文件自动选路
- 域名必需：没有 Active 域名不能发布或使用；绑定后才开放主人账号、网盘和分享
- 分享：支持随机令牌、1/7/30 天或长期有效、下载次数上限、分享管理、撤销与公开流式下载
...
1. **打开网盘【已配置完毕／缺少域名／尚未配置】**：已绑定域名时直接打开线上网盘；缺少域名时打开域名配置页，不能启动本机版。
2. **配置／重新配置**：首次使用或需要换桶、数据库、域名时打开中文安装助手。
3. **一键卸载**：先用本机随机安装凭证核对 Worker、R2 和 exact D1，再永久删除当前实例；安装助手创建的桶会删除，明确复用的桶只清理本实例 owner UUID 前缀并保留桶。若 R2 返回 `10008`，会启动临时远程清理 Worker，清除普通对象和残留分片上传后重试。
...
1. 连接自己的 Cloudflare。
2. 点击“准备我的网盘”。
3. 必须先发布并绑定 Active 域名，再打开域名网盘创建主人账号。

没有付费域名时，向导会提供 DPDNS 免费域名申请、Nameserver 回填和 Cloudflare 托管教程。Cloudflare Zone 变成 Active 后，回到向导重新识别并一键发布。

以后双击启动器选择第 1 项即可进入线上网盘；不需要查看或输入 localhost 地址。如果电脑没有 Node.js 22.13 或更新版本，启动器会打开 Node.js 官方下载页面并说明下一步。

第 3 项只处理当前 `wrangler.jsonc` 和 `.wrangler/primary-ownership.json` 共同证明的实例。执行前会显示 Worker、桶、数据库和域名，并要求手动输入 `DELETE`；随后核对 Cloudflare 账号、exact D1 编号、R2 随机对象标记及创建时间，并让线上 Worker 回答本机 HMAC 随机挑战。任何证据缺失或不符都会在删除前整体停止，保留 D1 和本机恢复依据。创建 R2 后即使配置中途退出，启动器也能按同一凭证安全处理该桶或移除复用桶中的标记。

受管 R2 桶删除后，其中的全部对象、CORS、生命周期、事件通知和其他桶级配置都会一并删除，无法恢复；复用桶不会被删除。Worker 的版本、Secret 和域名路由也会随 Worker 删除。任一步失败时，启动器都会保留 `wrangler.jsonc` 和随机归属凭证以便重试；它不会退出 Wrangler 登录，不会删除 Cloudflare 账号、域名专区或其他项目，也不会代替你删除在 Cloudflare 后台手动创建、可能被其他项目共用的 R2 API Token。

## 升级现有网盘

升级只替换程序并应用新增的 D1 迁移，不会清空原来的 R2 文件、主人账号、目录、分享或域名绑定。不要选择启动器中的“一键卸载”。
...
- 必须选择自己是否已有 Cloudflare 账号，以及是否已为 R2 绑定付款卡；未绑卡时先进入 R2 控制台处理，确认后可在下一页查找或一键创建私人 APAC R2 桶
...
向导不会把账号凭据发给项目作者，也不会复用任何预设项目 ID。域名识别只使用本机 Wrangler 登录授权向 Cloudflare 官方 API 读取 Zone 名称，Token 不会显示在页面或写入日志。发布与使用都强制要求已接入当前 Cloudflare 账号并处于 Active 状态的域名，同时关闭 `workers.dev`；没有域名时不会开放本机网盘入口。没有付费域名的用户可以按 [DPDNS 免费域名接入 Cloudflare](docs/free-domain-dpdns.md) 申请并托管免费名称。完整流程、命令白名单与手动安装方法见 [本地安装手册](docs/local-setup.md)。
...
管理员可在“个人设置 → 上传加速”点击一次完成常用优化：页面先把当前账号切换为 80 MiB 高吞吐分片和 6 路并发，再由只监听本机的助手使用 Wrangler 开启并复核 Local Uploads、同步 CORS。该流程不会打开 Cloudflare 页面，也不会要求复制 R2 密钥；如果实例原本已经配置直传 Secret，`auto` 模式仍会优先直传。
...
管理员可在“个人设置 → 多账号存储池”连接自己拥有并获准使用的其他 Cloudflare
账号。目标账号须先启用 R2，并完成 Cloudflare 要求的付款方式设置；Wrangler
无法代办账单。此后连接助手只使用 Wrangler：

1. 通过 Cloudflare 官方 OAuth 选择或登录目标账号。
2. 检查或创建该账号的私人 R2 桶并开启 Local Uploads。
3. 部署一个只绑定该桶的轻量 Storage Node Worker。
4. 使用一次性登记令牌和 P-256 签名完成主网盘与节点的双向身份校验。

附加账号的 R2 Access Key、Secret、账号密码和 MFA 不会保存到 D1 或浏览器。主网盘只保存节点地址、桶名、软容量和运行状态；签名私钥仅作为主 Worker Secret 保存，节点只取得公钥。

如果选择复用已有桶，助手会为该桶开启 Local Uploads；这是桶级设置，暂停节点写入或卸载 R2 Drive 时会保留，避免覆盖桶的其他配置。当前版本不允许只在网页删除节点：可以暂停新写入，或在本机启动器执行一键全卸载，让本机清单、Worker、R2 和 D1 保持一致。助手创建的新桶和 Worker 会写入本机受保护的 provisioning 清单，即使中途退出，一键卸载也能继续识别并清理。

存储池采用整文件分配：一个文件的全部 multipart 分片始终位于同一节点，D1 的 `storage_node_id` 负责把下载、分享、回收站和删除操作路由回正确账号。旧版已有文件继续留在主桶，不会在升级时搬动。附加节点离线时，已经位于该节点的文件会明确报错；系统不会从其他桶返回同名对象。新的上传在远端节点不可用时可回退主桶。

这里的“软容量”是 R2 Drive 的写入预算，用于调度和防止意外费用，不是 Cloudflare 报表中的实时剩余空间。R2 Standard 单桶没有固定总容量上限，免费用量和超额计费仍按每个 Cloudflare 账号分别计算。此功能用于统一管理、故障域隔离或成本分层，不得用于规避 Cloudflare 的服务限制、配额或条款。

## 文档

- [配置手册](docs/configuration.md)
- [本地安装手册](docs/local-setup.md)
- [DPDNS 免费域名接入 Cloudflare](docs/free-domain-dpdns.md)
