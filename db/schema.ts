...
export const files = sqliteTable(
  "files",
  {
    ...
    status: text("status", { enum: ["uploading", "ready", "failed", "deleted"] }).notNull(),
    createdAt: text("created_at").notNull(),
    updatedAt: text("updated_at").notNull(),
...
export const multipartUploads = sqliteTable(
  ...
  (table) => [uniqueIndex("multipart_file_idx").on(table.fileId), index("multipart_expiry_idx").on(table.expiresAt)],
);
